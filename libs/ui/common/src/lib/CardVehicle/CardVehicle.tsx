...
export function CardVehicle({
  ...
  return (
    <common.Card
      compact
      className={`hover:bg-gray-200 !bg-gray-100 my-2 text-gray-600 cursor-pointer ${className}`}
    >
      <div>
        <div className="flex">
          <div className=" flex items-center justify-center mr-1 relative">
            <span
              className={`w-3 h-3 ${statsColor} rounded-full absolute -ml-11`}
            />
          ...
          <div className="">
            ...
            <div className="flex justify-between">
              ...
              <div className="flex !items-center !justify-center">
                <div
                  className={`flex justify-center w-20 text-xs border-solid border-2 text-${velocityColor}-500 border-${velocityColor}-200 px-2 py-1 rounded-md bg-${velocityColor}-100 font-semibold`}
                >
                  <span>{Math.floor(Number(vehicle.speed))}</span>{' '}
                  <span>km/h</span>
                </div>
              </div>
            </div>
            <div className="flex justify-end mt-4">
              <button
                className="col-span-3 justify-center items-center flex bg-gray-700 rounded-sm text-gray-100 px-2 py-1 hover:bg-gray-600"
                onClick={() => {
