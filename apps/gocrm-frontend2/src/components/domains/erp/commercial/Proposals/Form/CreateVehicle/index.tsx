...
const CreateProposal = ({
    ...
    let servicosData = filteredItensGroups.map((item) => {
      if (data['tipo' + item].key === 'services') {
        return {
          ...
          ServicosPreco_Id:
            data['item' + item].key.PrestadoresDeServicos[0].Precos[0].Id,
          Veiculo: vehicleNumber
        }
      }
    })
  ...
  function getItemPrice(item: any, type: string) {
    ...
    switch (type) {
      ...
      case 'products':
        ...
        break
      case 'services':
        price = item.PrestadoresDeServicos[0].Precos[0].Valor
        break
      case 'combos':
  ...
  function getTotalValue() {
    ...
    filteredItensGroups.map((item) => {
      if (watch('item' + item)) {
        ...
        if (watch('tipo' + item).key === 'services') {
          totalPrice += Number(
            watch('item' + item).key.PrestadoresDeServicos[0].Precos[0].Valor
          )
        }
        if (watch('tipo' + item).key === 'combos') {
  ...
  useEffect(() => {
    ...
    if (productsData) {
      const filteredProducts = productsData
        ...
        .filter((product) => product.Fornecedores[0].Precos.length > 0)
    ...
    if (servicesData) {
      const filteredServices = servicesData
        ...
        .filter((service) => service.PrestadoresDeServicos[0].Precos.length > 0)
      setServicesArray(
  ...
  return (
    <common.Card>
        ...
        (itemPosition, index) =>
          itemPosition !== 0 && (
            <form.FormLine position={index} grid={1} key={index}>
              <div
                className={`grid ${inLineSubmit ? 'grid-cols-10' : 'grid-cols-9'
                  } gap-4`}
              >
                <Controller
                  control={control}
                            ...
                            await createProposalService({
                              variables: {
                                ...
                                ServicosPreco_Id: watch('item' + itemPosition)
                                  .key.PrestadoresDeServicos[0].Precos[0].Id,
                                Veiculo_Id: vehicleId,
                                Veiculo: vehicleNumber
                              }
                            }).then(() => proposalRefetch())
      ...
      {!inLineSubmit && (
        <div
          className={`flex items-center ${hideBackButton ? 'justify-end' : 'justify-between'
            } w-full px-6 mt-4`}
        >
