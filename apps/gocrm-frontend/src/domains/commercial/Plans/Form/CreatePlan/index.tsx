...
const CreatePlan = () => {
    ...
    try {
      const conditionalsValues = conditionalsGroup
        ...
        .map((conditional) => {
          ...
          return {
            Valor: utils.BRLMoneyUnformat(data['Valor' + conditional]),
      ...
      const precoServico =
        data.Servico_Id.key.PrestadoresDeServicos[0].Precos[0].Valor
      ...
      await createPlan({
        variables: {
          ...
          Nome: data.Nome,
          ServicoPreco_Id:
            data.Servico_Id.key.PrestadoresDeServicos[0].Precos[0].Id,
          ValorBase: Number(utils.BRLMoneyUnformat(data.Valor)),
          ValorPraticado: Number(ValorPraticado?.toFixed(2)) || null,
          data: conditionalsValues
        }
      }).then(() => {
        router.push(rotas.comercial.planos.index)
          utils.notification('Plano criado com sucesso', 'success')
      })
    } catch (err: any) {
      utils.showError(err)
    }
  ...
  function getTotalPrice() {
    ...
    if (watch('Valor')) {
      totalPrice += Number(utils.BRLMoneyUnformat(watch('Valor')))
    }
  ...
  function getPraticePrice() {
    if (watch('Servico_Id') && watch('Desconto')) {
      const value =
        watch('Servico_Id').key.PrestadoresDeServicos[0].Precos[0].Valor
      return utils.BRLMoneyInputDefaultFormat(        (value - value * (watch('Desconto') / 100)).toFixed(2)
      )
    }
  }
        ...
        <common.form.FormLine grid={3} position={1}>
            ...
            render={({ field: { onChange, value } }) => (
              <div>
                ...
                <common.OpenModalLink
                  onClick={() => router.push(rotas.comercial.servicos)}
                >
                  Cadastrar serviço
                </common.OpenModalLink>
              </div>
          ...
          <div>
            <common.form.Input
              ...
              value={utils.BRLMoneyInputDefaultFormat(                watch(
                  'Servico_Id'
                )?.key.PrestadoresDeServicos[0].Precos[0].Valor.toFixed(2) ||
                '0'
              )}
              icon="R$"
              disabled
                  ...
                  render={({ field: { onChange, value } }) => (
                    <div className="col-span-4">
                      ...
                      <common.OpenModalLink
                        onClick={() =>
                          router.push(
                            rotas.comercial.cadastros.condicionais
                          )
                        }
                      >
                  ...
                  render={({ field: { onChange, value } }) => (
                    <div className="col-span-4">
                      <common.form.Input
                        ...
                        title={`Valor ${watch('Condicional_Id' + conditionalPosition) !==
                            undefined
