...
const UpdatePlan = () => {
  ...
  async function onSubmit(data: any) {
    try {
      ...
      if (
        data.Valor !==
        utils.BRLMoneyInputDefaultFormat(          plansData?.Precos?.[0].ValorBase?.toString() as string
        ) ||
        data.ValorPraticado !==
        utils.BRLMoneyInputDefaultFormat(          plansData?.Precos?.[0].ValorPraticado?.toString() as string
        )
      ) {
  ...
  function getDefaultValue(index: number) {
    if (
      ...
      return utils.BRLMoneyInputDefaultFormat(        plansData?.Condicionais[index].Valor.toString() as string
      )
    }
        ...
        <common.form.FormLine grid={3} position={1}>
          ...
          <div>
            <common.form.Input
              ...
              value={utils.BRLMoneyInputDefaultFormat(                Number(plansData?.Precos[0].ServicoPreco.Valor).toFixed(2) ||
                '0'
              )}
              icon="R$"
              disabled
