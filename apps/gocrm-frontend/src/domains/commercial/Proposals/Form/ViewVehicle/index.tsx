...
const ViewVehicle = ({
  ...
  const [vehicleExits, setVehicleExits] = useState(false)
    ...
    proposalData?.Planos.map(async (plan, index) => {
      await updateProposalPlan({
        variables: {
          ...
          Id: plan.Id,
          Veiculo_Id: plan.Veiculo_Id || null
        }
      })
    })
    ...
    proposalData?.Produtos.map(async (product, index) => {
      await updateProposalProduct({
        variables: {
          ...
          Id: product.Id,
          Veiculo_Id: product.Veiculo_Id || null
        }
      })
    })
    ...
    proposalData?.Servicos.map(async (service, index) => {
      await updateProposalService({
        variables: {
          ...
          Id: service.Id,
          Veiculo_Id: service.Veiculo_Id || null
        }
      })
    })
    ...
    proposalData?.Combos.map(async (combo, index) => {
      await updateProposalCombo({
        variables: {
          ...
          Id: combo.Id,
          Veiculo_Id: combo.Veiculo_Id || null
        }
      })
    })
                    ...
                    <div key={item.Id}>
                      ...
                      <div className="flex items-center justify-between gap-4 mt-2">
                        <div className="flex gap-4">
                          <p>{item.Nome}</p>
                          <p>{utils.BRLMoneyFormat(item.Valor)}</p>
                        </div>
                        {activeEdit && (
                          <common.form.Switch
                            onChange={() => {
  ...
  async function removeItem(type: string, item: any) {
    try {
      ...
      setActiveEdit(false)
      refetchArraysData()
        utils.notification(itemName + ' excluido com sucesso', 'success')
    } catch (error: any) {
  ...
  async function linkVehicle() {
    ...
    refetchArraysData()
    proposalRefetch()
      utils.notification('Veículo vinculado com sucesso', 'success')
    setActiveEdit(false)
  }

  async function getVehicleDefaultValue() {
    if (proposalData?.Planos[0]?.Veiculo_Id) {
      const planVehicle = await getVehicleById(
        proposalData?.Planos[0]?.Veiculo_Id
      )
      setValue('Veiculo_Id123' + vehicle.position, {
        key: planVehicle?.data?.Id,
        title:
          (planVehicle?.data?.Placa !== null
            ? planVehicle.data?.Placa
            : planVehicle.data?.NumeroDoChassi?.substring(0, 10)) +
          ' - ' +
          planVehicle?.category?.Nome
      })
      return
    }

    if (proposalData?.Produtos[0]?.Veiculo_Id) {
      const productVehicle = await getVehicleById(
        proposalData?.Produtos[0]?.Veiculo_Id
      )
      setValue('Veiculo_Id123' + vehicle.position, {
        key: productVehicle?.data?.Id,
        title:
          (productVehicle?.data?.Placa !== null
            ? productVehicle.data?.Placa
            : productVehicle.data?.NumeroDoChassi?.substring(0, 10)) +
          ' - ' +
          productVehicle?.category?.Nome
      })
      return
    }

    if (proposalData?.Servicos[0]?.Veiculo_Id) {
      const serviceVehicle = await getVehicleById(
        proposalData?.Servicos[0]?.Veiculo_Id
      )
      setValue('Veiculo_Id123' + vehicle.position, {
        key: serviceVehicle?.data?.Id,
        title:
          (serviceVehicle?.data?.Placa !== null
            ? serviceVehicle.data?.Placa
            : serviceVehicle.data?.NumeroDoChassi?.substring(0, 10)) +
          ' - ' +
          serviceVehicle?.category?.Nome
      })
      return
    }

    if (proposalData?.Combos[0]?.Veiculo_Id) {
      const comboVehicle = await getVehicleById(
        proposalData?.Combos[0]?.Veiculo_Id
      )
      setValue('Veiculo_Id123' + vehicle.position, {
        key: comboVehicle?.data?.Id,
        title:
          (comboVehicle?.data?.Placa !== null
            ? comboVehicle.data?.Placa
            : comboVehicle.data?.NumeroDoChassi?.substring(0, 10)) +
          ' - ' +
          comboVehicle?.category?.Nome
      })
      return
    }

    if (proposalData?.Oportunidades[0]?.Veiculo_Id) {
      const upSellingVehicle = await getVehicleById(
        proposalData?.Oportunidades[0]?.Veiculo_Id
      )
      setValue('Veiculo_Id123' + vehicle.position, {
        key: upSellingVehicle?.data?.Id,
        title:
          (upSellingVehicle?.data?.Placa !== null
            ? upSellingVehicle.data?.Placa
            : upSellingVehicle.data?.NumeroDoChassi?.substring(0, 10)) +
          ' - ' +
          upSellingVehicle?.category?.Nome
      })
      return
    }
        ...
        const oldUpSellings = proposalData!.Oportunidades.map((upSelling) => {
          return {
            ...
            OportunidadeServico_Id: upSelling.OportunidadeDeServico
              ...
              : null,
            Veiculo: upSelling.Veiculo,
            Veiculo_Id: upSelling.Veiculo_Id as string | null
          }
        })
  ...
  useEffect(() => {
    if (vehiclesData) {
      const filteredVehicles = vehiclesData
        .filter(
          (vehicle) =>
            !licensePlates.includes(vehicle.Placa as string) &&
            vehicle.VeiculosAtivos.length === 0
        )
        .map(async (vehicle) => {
          const category = await getVehicleCategoryById(vehicle.Categoria_Id)
          return {
            Id: vehicle.Id,
            Cliente: vehicle.Cliente,
            NumeroDoChassi: vehicle.NumeroDoChassi,
            Placa: vehicle.Placa,
            Categoria: {
      ...
      ;(async () => {
        setUnselectedVehicles(await Promise.all(filteredVehicles))
      })()
    }
  ...
  useEffect(() => {
    if (router.query.origin) {
      if (router.query.origin === 'changeVehicle') {
        vehicle.Id !== null &&
          getActiveVehicleById(vehicle.Id as string).then((activeVehicles) => {
            if (
              (activeVehicles?.filter(
                (activeVehicle) => activeVehicle.Situacao.Valor === 'ativo'
              ).length || 0) > 0
            ) {
              setVehicleExits(true)
            }
          })
      }
    }
        ...
        <div className="flex items-start justify-center w-3/5 gap-4">
          ...
          {!vehicleExits ? (
            router.query.origin === 'changeVehicle' ? (
              <div className="flex justify-end flex-1">
                <p className="mr-6 text-xl">
                  Este veículo vai ser{' '}
                  <span className="text-primary-9">Ativado</span>
                </p>
              </div>
            ) : (
              <>
                            ...
                            ? unselectedVehicles.map((vehicle) => {
                                return {
                                  ...
                                  title:
                                    (vehicle.Placa
                                      ? vehicle.Placa
                                      : vehicle.NumeroDoChassi?.substring(
                                          0,
                                          10
                                        )) +
                                    ' - ' +
            ...
            <div className="flex justify-end flex-1">
              <p className="mr-6 text-xl">
                Este veículo vai ser{' '}
                <span className="text-primary-3">Desativado</span>
              </p>
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-9 gap-4">
            ...
            <div className="col-span-2">
              <common.form.Input
                fieldName={'Valor'}
                title={`Valor`}
                value={utils.BRLMoneyInputDefaultFormat(                  watch('item1' + plan) ? showPlanPrice(plan) : '0'
                )}
                icon="R$"
                disabled
              />
          ...
          <div className="grid grid-cols-9 gap-4">
            ...
            <div className="col-span-2">
              <common.form.Input
                fieldName={'Valor'}
                title={`Valor`}
                value={utils.BRLMoneyInputDefaultFormat(                  watch('item2' + product)
          ...
          <div className="grid grid-cols-9 gap-4">
            ...
            <div className="col-span-2">
              <common.form.Input
                fieldName={'Valor'}
                title={`Valor`}
                value={utils.BRLMoneyInputDefaultFormat(                  watch('item3' + service)
          ...
          <div className="grid grid-cols-9 gap-4">
            ...
            <div className="col-span-2">
              <common.form.Input
                fieldName={'Valor'}
                title={`Valor`}
                value={utils.BRLMoneyInputDefaultFormat(                  watch('item4' + combo) ? showComboPrice(combo) : undefined
                )}
                icon="R$"
                disabled
              />
          ...
          {!activeEdit &&
            ...
            router.query.origin === undefined && (
              <common.buttons.CancelButton
                ...
                title="Excluir veículo dessa proposta"
                disabled={disabledDeleteButtons()}
                loading={disabledDeleteButtons()}
              />
            )}
