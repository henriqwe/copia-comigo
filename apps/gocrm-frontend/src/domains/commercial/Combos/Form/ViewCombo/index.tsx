...
const ViewCombo = () => {
  ...
  async function createPlan(
    ...
    await createComboPlan({
      ...
      itensGroup[index] = 0
      setReload(!reload)
        utils.notification('Plano adicionado com sucesso', 'success')
    })
  }
  ...
  async function createProduct(
    ...
    await createComboProduct({
      ...
      itensGroup[index] = 0
      setReload(!reload)
        utils.notification('Produto adicionado com sucesso', 'success')
    })
  }
  ...
  async function createService(
    service: {
      Id: string
      PrestadoresDeServicos: { Precos: { Id: string; Valor: number }[] }[]
    },
    index: number,
    percentOff = undefined
  ) {
    let value = service.PrestadoresDeServicos[0].Precos[0].Valor
    if (percentOff) {
      value = value - value * (percentOff / 100)
    }
    await createComboService({
      variables: {
        ...
        ServicoPreco_Id: service.PrestadoresDeServicos[0].Precos[0].Id,
        ValorPraticado: value
      }
    }).then(() => {
      comboRefetch()
      itensGroup[index] = 0
      setReload(!reload)
        utils.notification('Serviço adicionado com sucesso', 'success')
    })
  }
  ...
  async function deletePlan(Id: string, index: number) {
    await softDeleteComboPlan({
      ...
      // })
        utils.notification('Plano deletado com sucesso', 'success')
    })
  }
  ...
  async function deleteProduct(Id: string, index: number) {
    await softDeleteComboProduct({
      ...
      //   setValue('Valor' + position, comboData?.Planos[index].Plano)
      // })
        utils.notification('Produto deletado com sucesso', 'success')
    })
  }
  ...
  async function deleteService(Id: string, index: number) {
    await softDeleteComboService({
      ...
      //   setValue('Valor' + position, comboData?.Planos[index].Plano)
      // })
        utils.notification('Serviço deletado com sucesso', 'success')
    })
  }
  ...
  function getItemPrice(item: any, type: string, position: number) {
    ...
    switch (type) {
      case 'plans':
        ...
        break
      case 'products':
        price = utils.BRLMoneyInputDefaultFormat(          item.Fornecedores[0].Precos[0].Valor.toFixed(2)
        ) as string

        break
      case 'services':
        price = utils.BRLMoneyInputDefaultFormat(          item.PrestadoresDeServicos[0].Precos[0].Valor.toFixed(2)
        ) as string
  ...
  function getPraticePriceValue(percentOff: number, position: number) {
    let finalValue = Number(utils.BRLMoneyUnformat(watch('Valor' + position)))
    finalValue = finalValue - finalValue * (percentOff / 100)
    setValue(
      'ValorPraticado' + position,
     utils.BRLMoneyInputDefaultFormat(finalValue.toFixed(2))
    )
  }
  ...
  useEffect(() => {
    ...
    if (servicesData) {
      const filteredServicesData = servicesData.filter(
        (service) => service.PrestadoresDeServicos.length > 0
      )
      setServicesArray(
  ...
  return (
    <common.Card>
      ...
      <common.Separator />
      <form>        <common.form.FormLine grid={2} position={1}>
          <div>
            <common.form.Input
              fieldName="Nome"
              title="Nome"
          ...
          <div>
            <common.form.Input
              fieldName={'Valor'}
              title={`Preço`}
              value={utils.BRLMoneyInputDefaultFormat(                Number(comboData?.Precos[0].ValorBase).toFixed(2)
              )}
              icon="R$"
              disabled
            />
              ...
              <div className="grid grid-cols-6 gap-4">
                ...
                <div className="col-span-2">
                  <common.form.Input
                    fieldName={'Valor'}
                    title={`Valor`}
                    value={utils.BRLMoneyInputDefaultFormat(                      Number(planPrice).toFixed(2)
                    )}
                    icon="R$"
                    disabled
                  />
                ...
                <div className="flex col-span-2 gap-2">
                  <div className="flex-1">
                    <common.form.Input
                      ...
                      value={utils.BRLMoneyInputDefaultFormat(                        Number(plan.ValorPraticado).toFixed(2)
                      )}
                      icon="R$"
                      disabled
                    />
            ...
            <div className="grid grid-cols-6 gap-4">
              ...
              <div className="col-span-2">
                <common.form.Input
                  fieldName={'Valor'}
                  title={`Valor`}
                  value={utils.BRLMoneyInputDefaultFormat(                    product.ProdutoPreco.Valor || '0'
                  )}
                  icon="R$"
                  disabled
                />
              ...
              <div className="flex col-span-2 gap-2">
                <div className="flex-1">
                  <common.form.Input
                    fieldName={'ValorPraticado'}
                    title="Valor praticado"
                    value={utils.BRLMoneyInputDefaultFormat(                      Number(product.ValorPraticado).toFixed(2)
                    )}
                    icon="R$"
                    disabled
                  />
            ...
            <div className="grid grid-cols-6 gap-4">
              ...
              <div className="col-span-2">
                <common.form.Input
                  fieldName={'Valor'}
                  title={`Valor`}
                  value={utils.BRLMoneyInputDefaultFormat(                    service.ServicosPreco.Valor || '0'
                  )}
                  icon="R$"
                  disabled
                />
              ...
              <div className="flex col-span-2 gap-2">
                <div className="flex-1">
                  <common.form.Input
                    fieldName={'ValorPraticado'}
                    title="Valor praticado"
                    value={utils.BRLMoneyInputDefaultFormat(                      Number(service.ValorPraticado).toFixed(2)
                    )}
                    icon="R$"
                    disabled
                  />
