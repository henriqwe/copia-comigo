...
const CreateCombo = () => {
  ...
  async function onSubmit(data: any) {
    try {
      ...
      await createCombo({
        ...
        router.push(rotas.comercial.combos.index)
  ...
  function getTotalPrice() {
          ...
          case 'plans':
            ...
            if (watch('Desconto' + item)) {
              value =
                value -
                value *
                  (Number(utils.BRLMoneyUnformat(watch('Desconto' + item))) / 100)
            }
            price += value
            break
          case 'products':
            ...
            break
          case 'services':
            value = watch('item' + item).key.PrestadoresDeServicos[0].Precos[0]
              .Valor
            if (watch('Desconto' + item)) {
              value =
                value -
    ...
    if (watch('Valor')) {
      price += Number(utils.BRLMoneyUnformat(watch('Valor')))
    }

    return utils.BRLMoneyFormat(Number(price.toFixed(2)))
  }
