...
const UpdatePurchaseOrder = () => {
  ...
  function loadDate() {
    if (purchaseOrderData?.DataEntrada) {
      setDate(utils.ptBRtimeStamp(purchaseOrderData?.DataEntrada as Date))
      return
    }
    if (purchaseOrderData?.DataEntregue) {
      setDate(utils.ptBRtimeStamp(purchaseOrderData?.DataEntregue as Date))
      return
    }
    if (purchaseOrderData?.DataCompra) {
      setDate(utils.ptBRtimeStamp(purchaseOrderData?.DataCompra as Date))
      return
    }
    if (purchaseOrderData?.DataAutorizacao) {
      setDate(utils.ptBRtimeStamp(purchaseOrderData?.DataAutorizacao as Date))
      return
    }
    if (purchaseOrderData?.DataOrcamento) {
      setDate(utils.ptBRtimeStamp(purchaseOrderData?.DataOrcamento as Date))
      return
    }
    setDate(utils.ptBRtimeStamp(purchaseOrderData?.DataAbertura as Date))
  }
